...
public class PwdAlt extends JDialog implements ActionListener {
	/**
	 * 管理员密码修改对话框
	 */
	...
	public void actionPerformed(ActionEvent e) {//设置监听器
		if(e.getSource() == jbSubmit) {
			if(jpfOldPwd.getText().length() == 0 ||jpfNewPwd.getText().length() == 0 || jlIdentify.getText().length() == 0) {
				...
				ResultSet rs = null;
				try {
					stm = con.prepareStatement("select managerPwd from manager where managerID='"+Login.getUsername()+"'");
					rs = stm.executeQuery();
					rs.next(); 
					if(rs.getString(1).equals(jpfOldPwd.getText())) {
						if(jpfNewPwd.getText().equals(jpfIdentify.getText())) {
							try {
								stm = con.prepareStatement("update manager set managerPwd='"+jpfNewPwd.getText()+"' where managerID='"+Login.getUsername()+"'");
								stm.executeUpdate();
